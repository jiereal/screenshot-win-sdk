...
extern  int32_t g_DesktopScale;//百分制
...
void DrawUnit::Render(HDC hdc, HBITMAP bitmap, bool is_continue, bool bScaled)
{
	m_bScaled = bScaled;
	if (m_bScaled)
	{
		left_ = left_*g_DesktopScale / 100;
		top_ = top_*g_DesktopScale / 100;
		right_ = right_ *g_DesktopScale / 100;
		bottom_ = bottom_*g_DesktopScale / 100;
	}
...
void DrawUnitBrush::RenderSelf(HDC hdc, HBITMAP bitmap, bool is_continue)
{
	...
	if (m_bScaled)
	{
		for (auto& it : m_vtPoints)
		{
			it.x = it.x*g_DesktopScale / 100;
			it.y = it.y*g_DesktopScale / 100;
		}
	}
...
void DrawUnitArrow::DrawArrow(HDC hdc)
{
	...
	//计算出水平时箭头三个点的坐标
	float diff_y = (float)(pt_end.y - pt_start.y);
	float diff_x = (float)(pt_end.x - pt_start.x);
	float radius = atan(diff_y/diff_x); // arc_tan(y/x)求角度
	diff_x = float((fabs(diff_x) <= 0.00001) ? (diff_x < 0 ? -0.00001 : 0.00001) :diff_x);
	...
	//进行旋转
	float x = (float)pt_start.x;
	float y = (float)pt_start.y;
