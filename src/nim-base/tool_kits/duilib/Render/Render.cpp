...
bool RenderContext_GdiPlus::Resize(int width, int height)
{
	ASSERT(m_hDC);
	...
	m_bitmap.Init(m_hDC, width, height);
...
void RenderContext_GdiPlus::ClearAlpha(const UiRect& rcDirty)
{
	m_bitmap.ClearAlpha(rcDirty);
}

void RenderContext_GdiPlus::RestoreAlpha(const UiRect& rcDirty, const UiRect& rcShadowPadding)
{
...
void RenderContext_GdiPlus::DrawImage(const UiRect& rcPaint, HBITMAP hBitmap, bool bAlphaChannel,
	const UiRect& rcImageDest, const UiRect& rcImageSource, const UiRect& rcCorners, BYTE uFade /*= 255*/, bool xtiled /*= false*/, bool ytiled /*= false*/, int nRandBottomMargin , std::vector<int> vecRandHeight)
{
	UiRect rcTestTemp;
    ...
    if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
        if( !xtiled && !ytiled ) {
			...
			UiRect rcPaintDest;
			rcPaintDest.top = rcDest.top;
			rcPaintDest.bottom = rcDest.bottom;
            ...
            for( int i = 0; i < iTimes; ++i ) {
				...
				rcPaintDest.left = lDestLeft;
				rcPaintDest.right = lDestRight;
				rcSource.left = rcImageSource.left + rcCorners.left;
				rcSource.top = rcImageSource.top + rcCorners.top;
				rcSource.right = rcSource.left + lDrawWidth;
				rcSource.bottom = rcImageSource.bottom - rcCorners.bottom;

				if (!vecRandHeight.empty())
				{
					rcPaintDest.bottom = rcDest.bottom - nRandBottomMargin;
					rcPaintDest.top = rcDest.bottom - nRandBottomMargin - vecRandHeight[i%vecRandHeight.size()];
				}

				DrawFunction(m_hDC, m_bTransparent, rcPaintDest, hCloneDC, rcSource, bAlphaChannel, uFade);
            }
        }
        else { // ytiled
            LONG lHeight = rcImageSource.bottom - rcImageSource.top - rcCorners.top - rcCorners.bottom;
			...
			UiRect rcPaintDest;
			rcPaintDest.left = rcDest.left;
			rcPaintDest.right = rcDest.right;
            ...
            for( int i = 0; i < iTimes; ++i ) {
				...
				rcPaintDest.top = lDestTop;
				rcPaintDest.bottom = lDestBottom;
				rcSource.left = rcImageSource.left + rcCorners.left;
				rcSource.top = rcImageSource.top + rcCorners.top;
				rcSource.right = rcImageSource.right - rcCorners.right;
				rcSource.bottom = rcSource.top + lDrawHeight;
...
void RenderContext_GdiPlus::DrawText(const UiRect& rc, const std::wstring& strText, DWORD dwTextColor, int iFont, UINT uStyle, BYTE uFade /*= 255*/, bool bLineLimit /*= false*/, bool bWaterMark)
{
	ASSERT(::GetObjectType(m_hDC)==OBJ_DC || ::GetObjectType(m_hDC)==OBJ_MEMDC);
	...
	if (bWaterMark)
	{
		Gdiplus::REAL angle = 325;
		graphics.TranslateTransform(rc.left + rc.GetWidth() / 2, rc.top + rc.GetHeight() / 2);
...
ui::UiRect RenderContext_GdiPlus::MeasureText(const std::wstring& strText, int iFont, UINT uStyle, bool bMeasureTrailingSpaces, int width /*= DUI_NOSET_VALUE*/)
{
	Gdiplus::Graphics graphics(m_hDC);
	Gdiplus::Font font(m_hDC, GlobalManager::GetFont(iFont));
	Gdiplus::RectF bounds;
