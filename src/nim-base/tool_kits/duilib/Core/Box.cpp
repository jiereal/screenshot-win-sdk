...
Box::Box(Layout* pLayout) :
	...
	m_uSepState(kControlStateNormal),
	m_items(),
	OnBubbledEvent()
{
...
void Box::SetAttribute(const std::wstring& strName, const std::wstring& strValue)
{
	...
	else if (strName == _T("sepsize")) SetSepSize(_ttoi(strValue.c_str()));
	else if (strName == _T("septype"))
	{
...
void Box::HandleMessage(EventArgs& msg)
{
	...
	if (msg.Type == kEventSetCursor)
	{
		if (IsEnabled())
		{
			if (IsInSepHorArea(msg.ptMouse))
			{
				::SetCursor(::LoadCursor(NULL, MAKEINTRESOURCE(IDC_SIZEWE)));
				return;
			}
			else if (IsInSepVerArea(msg.ptMouse))
			{
	...
	{
		if (m_uSepState == kControlStatePushed)
		{
			DragSep(msg.ptMouse);
		}
...
bool Box::ButtonDown(EventArgs& msg)
{
	bool bRet = __super::ButtonDown(msg);
	if (bRet)
	{
		if (IsInSepArea(kSepLeft, msg.ptMouse))
		{
			m_nSepTypeDrag = kSepLeft;
			m_uSepState = kControlStatePushed;
		}
		else if (IsInSepArea(kSepRight, msg.ptMouse))
		{
			m_nSepTypeDrag = kSepRight;
			m_uSepState = kControlStatePushed;
		}
		else if (IsInSepArea(kSepTop, msg.ptMouse))
		{
			m_nSepTypeDrag = kSepTop;
			m_uSepState = kControlStatePushed;
		}
		else if (IsInSepArea(kSepBottom, msg.ptMouse))
		{
			m_nSepTypeDrag = kSepBottom;
			m_uSepState = kControlStatePushed;
		}
	}
...
bool Box::ButtonUp(EventArgs& msg)
{
...
void Box::Paint(IRenderContext* pRender, const UiRect& rcPaint)
{
	UiRect rcTemp;
...
ui::Control* Box::GetItemByName(const std::wstring& strName)
{
	if (strName.empty())
		return NULL;
...
ui::Control* Box::GetItemByUTF8Name(const std::string& strName)
{
	if (strName.empty())
		return NULL;
...
ui::Control* Box::GetItemByDataID(const std::wstring& strText)
{
	if (strText.empty())
		return NULL;
...
ui::Control* Box::GetItemByUTF8DataID(const std::string& strText)
{
	if (strText.empty())
		return NULL;
...
void Box::GetItemByUTF8DataID(const std::string& strText, std::list<Control*>& m_listItem)
{
	if (strText.empty())
		return;
...
void Box::RemoveAll()
{
	if (m_bAutoDestroy) {
		for (auto it = m_items.begin(); it != m_items.end(); it++) {
			...
			else if (!(*it)->GetWeakFlag().expired())
			{
				delete (*it);
			}
		}
...
int Box::GetSepSize(void) const
{
	return m_nSepSize;
}

void Box::SetSepSize(int nSize)
{
	if (0 > nSize)
		return;
...
int Box::GetSepType(void) const
{
	return m_nSepType;
}

void Box::SetSepType(int nType)
{
	if (0 > nType)
		return;

	if ((kSepBottom << 1) <= nType)
	{
		m_nSepType = (kSepBottom << 1) - 1;
	}
	else
	{
		m_nSepType = nType;
...
void Box::Refresh()
{
	GetLayout()->ArrangeChild(m_items, m_rcItem);
}

bool Box::IsInSepArea(SepType eSepType, const POINT& ptMouse)
{
	if (0 == GetSepSize())
		return false;

	if ((GetSepType() & eSepType) != eSepType)
		return false;

	if ((eSepType & kSepLeft) == kSepLeft)
	{
		UiRect rcPos;
		rcPos.left = m_rcItem.left;
		rcPos.top = m_rcItem.top;
		rcPos.right = m_rcItem.left + GetSepSize();
		rcPos.bottom = m_rcItem.bottom;

		return rcPos.IsPointIn(ptMouse);
	}
	else if ((eSepType & kSepRight) == kSepRight)
	{
		UiRect rcPos;
		rcPos.left = m_rcItem.right - GetSepSize();
		rcPos.top = m_rcItem.top;
		rcPos.right = m_rcItem.right;
		rcPos.bottom = m_rcItem.bottom;

		return rcPos.IsPointIn(ptMouse);
	}
	else if ((eSepType & kSepTop) == kSepTop)
	{
		UiRect rcPos;
		rcPos.left = m_rcItem.left;
		rcPos.top = m_rcItem.top;
		rcPos.right = m_rcItem.right;
		rcPos.bottom = m_rcItem.top + GetSepSize();

		return rcPos.IsPointIn(ptMouse);
	}
	else if ((eSepType & kSepBottom) == kSepBottom)
	{
		UiRect rcPos;
		rcPos.left = m_rcItem.left;
		rcPos.top = m_rcItem.bottom - GetSepSize();
		rcPos.right = m_rcItem.right;
		rcPos.bottom = m_rcItem.bottom;
...
bool Box::IsInSepHorArea(const POINT& ptMouse)
{
	return IsInSepArea(kSepLeft, ptMouse) || IsInSepArea(kSepRight, ptMouse);
}

bool Box::IsInSepVerArea(const POINT& ptMouse)
{
	return IsInSepArea(kSepTop, ptMouse) || IsInSepArea(kSepBottom, ptMouse);
}

int Box::GetSepWidth(void) const
{
	int iWidth = 0;
	if (m_nSepType & kSepLeft)
	{
		iWidth += GetSepSize();
	}

	if (m_nSepType & kSepRight)
	{
		iWidth += GetSepSize();
	}
...
int Box::GetSepHeight(void) const
{
	int iHeight = 0;
	if (m_nSepType & kSepTop)
	{
		iHeight += GetSepSize();
	}

	if (m_nSepType & kSepBottom)
	{
		iHeight += GetSepSize();
	}
...
void Box::DragSep(const POINT& ptMouse)
{
	Box *pBoxParent = GetParent();
	UiRect rcWnd = GetWindow()->GetPos(false);
	UiRect rcPos = m_rcItem;
	if (m_nSepTypeDrag == kSepLeft)
	{
		if (m_rcItem.right - ptMouse.x < GetMinWidth())
		{
			rcPos.left = m_rcItem.right - GetMinWidth();
		}
		else if (m_rcItem.right - ptMouse.x > GetMaxWidth())
		{
			rcPos.left = m_rcItem.right - GetMaxWidth();
		}
		else
		{
			rcPos.left = ptMouse.x;
		}

		if (NULL != pBoxParent)
		{
			UiRect rcParent = pBoxParent->GetPos();
			if (rcPos.left < rcParent.left)
			{
				rcPos.left = rcParent.left;
			}
		}
		...
		if (rcPos.left + GetSepWidth() > rcPos.right)
		{
			rcPos.left = rcPos.right - GetSepWidth();
		}

		SetFixedWidth(rcPos.GetWidth());
	}
	else if (m_nSepTypeDrag == kSepRight)
	{
		if (ptMouse.x - m_rcItem.left < GetMinWidth())
		{
			rcPos.right = m_rcItem.left + GetMinWidth();
		}
		else if (ptMouse.x - m_rcItem.left > GetMaxWidth())
		{
			rcPos.right = m_rcItem.left + GetMaxWidth();
		}
		else
		{
			rcPos.right = ptMouse.x;
		}

		if (NULL != pBoxParent)
		{
			UiRect rcParent = pBoxParent->GetPos();
			if (rcPos.right > rcParent.right)
			{
				rcPos.right = rcParent.right;
			}
		}

		if (rcPos.right > rcWnd.GetWidth())
		{
			rcPos.right = rcWnd.GetWidth();
		}

		if (rcPos.right - GetSepWidth() < rcPos.left)
		{
			rcPos.right = rcPos.left + GetSepWidth();
		}

		SetFixedWidth(rcPos.GetWidth());
	}
	if (m_nSepTypeDrag == kSepTop)
	{
		if (m_rcItem.bottom - ptMouse.y < GetMinHeight())
		{
			rcPos.top = m_rcItem.bottom - GetMinHeight();
		}
		else if (m_rcItem.bottom - ptMouse.y > GetMaxHeight())
		{
			rcPos.top = m_rcItem.bottom - GetMaxHeight();
		}
		else
		{
			rcPos.top = ptMouse.y;
		...
		{
			UiRect rcParent = pBoxParent->GetPos();
			if (rcPos.top < rcParent.top)
			{
				rcPos.top = rcParent.top;
			}
		}
		...
		if (rcPos.top + GetSepHeight() > rcPos.bottom)
		{
			rcPos.top = rcPos.right - GetSepHeight();
		}

		SetFixedHeight(rcPos.GetHeight());
	}
	else if (m_nSepTypeDrag == kSepBottom)
	{
		if (ptMouse.y - m_rcItem.top < GetMinHeight())
		{
			rcPos.bottom = m_rcItem.top + GetMinHeight();
		}
		else if (ptMouse.y - m_rcItem.top > GetMaxHeight())
		{
			rcPos.bottom = m_rcItem.top + GetMaxHeight();
		}
		else
		{
			rcPos.bottom = ptMouse.y;
		}

		if (NULL != pBoxParent)
		{
			UiRect rcParent = pBoxParent->GetPos();
			if (rcPos.bottom > rcParent.bottom)
			{
				rcPos.bottom = rcParent.bottom;
			}
		}

		if (rcPos.bottom > rcWnd.GetHeight())
		{
			rcPos.bottom = rcWnd.GetHeight();
		}

		if (rcPos.bottom - GetSepHeight() < rcPos.top)
		{
			rcPos.bottom = rcPos.top + GetSepHeight();
		}
...
void ScrollableBox::HandleMessage(EventArgs& event)
{
	...
	if( m_pVerticalScrollBar != NULL && m_pVerticalScrollBar->IsValid() && m_pVerticalScrollBar->IsEnabled() ) {
		...
		else if (event.Type == kEventTouchDown) {
			m_ptLastTouchPos = event.ptMouse;
		}
		else if (event.Type == kEventTouchMove) {
		...
		else if (event.Type == kEventTouchUp) {
			m_ptLastTouchPos = event.ptMouse;
		}
	}
...
void ScrollableBox::Paint(IRenderContext* pRender, const UiRect& rcPaint)
{
	UiRect rcTemp;
	if( !::IntersectRect(&rcTemp, &rcPaint, &m_rcItem) ) return;
...
{
	CSize scrollPos = GetScrollPos();
	auto itNewBegin = m_items.begin();
	auto itNewEnd = m_items.end();
	if (!bFromTopLeft) {
		auto itNewBegin = m_items.rbegin();
		auto itNewEnd = m_items.rend();
	}
	for (auto it = itNewBegin; it != itNewEnd; it++) {
		Control* pControl = *it;
		if (!pControl->IsVisible()) continue;
...
void ScrollableBox::TouchUp(int detaValue)
{
	CSize scrollPos = GetScrollPos();
	if (scrollPos.cy <= 0) {
		return;
	...
	scrollPos.cy = scrollPos.cy - detaValue;
	SetScrollPos(scrollPos);
}

void ScrollableBox::TouchDown(int detaValue)
{
	CSize scrollPos = GetScrollPos();
	if (scrollPos.cy >= GetScrollRange().cy) {
		return;
