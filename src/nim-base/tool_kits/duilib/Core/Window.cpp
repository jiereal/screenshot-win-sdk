...
UINT Window::ShowModal()
{
    ...
    while( ::IsWindow(hTempWnd) && ::GetMessage(&msg, NULL, 0, 0) ) {
		if (msg.message == WM_CLOSE && msg.hwnd == hWndParent)
		{
			nRet = IDCLOSE;
			break;
		}
    ...
    ::EnableWindow(hWndParent, TRUE);
    ::SetFocus(hWndParent);
	if (msg.message == WM_CLOSE && msg.hwnd == hWndParent) ::PostMessage(hWndParent, msg.message, msg.wParam, msg.lParam);
		...
		if (nNumInputs >= 1 && GetTouchInputInfoWrapper((HTOUCHINPUT)lParam, nNumInputs, pInputs, sizeof(TOUCHINPUT)))
		{
			if (pInputs[0].dwID != 0)
			{
				...
				if (pInputs[0].dwFlags & TOUCHEVENTF_DOWN)
				{
					...
					pControl->HandleMessageTemplate(kEventTouchDown, wParam, lParam, 0, pt);
				...
				else if (pInputs[0].dwFlags & TOUCHEVENTF_UP)
				{
					...
					m_pEventTouch->HandleMessageTemplate(kEventTouchUp, wParam, lParam, 0, pt);
					m_pEventTouch = NULL;
				}
			}
		}
...
void Window::Paint()
{
	...
	if (m_shadow.IsShadowAttached() && m_renderOffset.x == 0 && m_renderOffset.y == 0) {
		...
		if (m_shadow.IsUserDefaultImage())
		{
			//考虑圆角
		...
		else
		{
			...
			{
				rcNewPaint.Deflate(rcAlphaFixCorner);
			}
		}
