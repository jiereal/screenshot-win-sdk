...
void GlobalManager::Startup(const std::wstring& strResourcePath, const CreateControlCallback& callback, bool bAdaptDpi, const std::wstring& strLangPath, const std::wstring& language)
{
	...
	if (strLanguageSetting.empty())
	{
		...
		if (lcid == 0x804)
		{
			strLanguageSetting = L"zh_CN";
		}
		else
		{
			strLanguageSetting = L"en_US";
		}
	}
...
{
	return std::make_unique<ui::RenderContext_GdiPlus>();
}
...
DWORD GlobalManager::GetTextColor(const std::wstring& strName)
{
	...
	else
	{
		...
		LPTSTR pstr = NULL;
		DWORD dwColor = _tcstoul(strColor.c_str(), &pstr, 16);
		return dwColor;
	}
}
...
std::shared_ptr<ImageInfo> GlobalManager::GetImage(const std::wstring& bitmap)
{
	...
	auto it = m_mImageHash.find(imageFullPath);
	if (it == m_mImageHash.end()) {
		std::unique_ptr<ImageInfo> data = ImageInfo::LoadImage(imageFullPath);
