...
INT_PTR Control::GetTag(void) const
{
	return m_iTag;
}

void Control::SetTag(INT_PTR iTag)
{
	m_iTag = iTag;
}
...
void Control::SetAttribute(const std::wstring& strName, const std::wstring& strValue)
{
	...
	else if (strName == _T("rand"))
	{
		std::list<std::wstring> splitList = StringHelper::Split(strValue, L",");
		std::vector<int>  vecValue;
		for (auto& it : splitList)
		{
			vecValue.push_back(_ttoi(it.c_str()));
		}
		if (!splitList.empty())
		{
			for (int i = 0; i < 50; i++)
			{
				m_vecRandHeight.push_back(vecValue[rand() % splitList.size()]);
			}
		}
	}
	else if (strName == _T("randbottommargin"))
	{
		m_RandbottomMargin = _ttoi(strValue.c_str());
	}
	...
	else if (strName == _T("watermark"))	SetWaterMarkEnable(strValue == _T("true"));
	else if (strName == _T("watermarkhorinterval"))  SetWaterMarkHorInterval(_ttoi(strValue.c_str()));
	else if (strName == _T("watermarkverinterval"))  SetWaterMarkVerInterval(_ttoi(strValue.c_str()));
	else if (strName == _T("watermarktext")) SetWaterMarkText(strValue);
	else if (strName == _T("watermarkcolor")) SetWaterMarkColor(strValue);
	else ASSERT(FALSE);
}
...
bool Control::DrawImage(IRenderContext* pRender, Image& duiImage, const std::wstring& strModify /*= L""*/, int nFade /*= DUI_NOSET_VALUE*/)
{
	...
	if (m_bkImage.imageCache && m_bkImage.imageCache->IsGif() && m_bGifPlay && !m_bkImage.IsPlaying()) {
		...
		pRender->DrawImage(m_rcPaint, duiImage.GetCurrentHBitmap(), imageInfo->IsAlpha(),
			rcNewDest, rcNewSource, newImageAttribute.rcCorner, iFade, newImageAttribute.bTiledX, newImageAttribute.bTiledY,m_RandbottomMargin, m_vecRandHeight);
	}
...
void Control::AlphaPaint(IRenderContext* pRender, const UiRect& rcPaint)
{
	...
	if (!IsAlpha()) {
		...
		auto pMemRender = GlobalManager::CreateRenderContext();
		pMemRender->Resize(size.cx, size.cy);

		{
			UiRect rcClip = { 0, 0, size.cx, size.cy };
			AutoClip alphaClip(pMemRender.get(), rcClip, m_bClip);
			AutoClip roundAlphaClip(pMemRender.get(), rcClip, m_cxyBorderRound.cx, m_cxyBorderRound.cy, bRoundClip);
			...
			CPoint ptOldOrg = pMemRender->OffsetWindowOrg(ptOffset);
			Paint(pMemRender.get(), m_rcItem);
			pMemRender->SetWindowOrg(ptOldOrg);
			m_pWindow->SetRenderTransparent(bOldCanvasTrans);

			pRender->AlphaBlend(m_rcItem.left, m_rcItem.top, m_rcItem.right - m_rcItem.left, m_rcItem.bottom - m_rcItem.top, pMemRender->GetDC(),
				0, 0, m_rcItem.right - m_rcItem.left, m_rcItem.bottom - m_rcItem.top, m_nAlpha);
		}
	}
}
...
void Control::PaintWaterMark(IRenderContext* pRender)
{
	if (m_EnableWaterMark && !m_strWarterMarkText.empty())
	{
		int nHorCount = (m_rcItem.right - m_rcItem.left) / m_nHorWarterMarkInterval;      //  m_nHorWarterMarkInterval*m_nVerWarterMarkInterval划分区域
		if (((m_rcItem.right - m_rcItem.left) % m_nHorWarterMarkInterval) != 0)
		{
			nHorCount = nHorCount + 1;
		}

		int nVerCount = (m_rcItem.bottom - m_rcItem.top) / m_nVerWarterMarkInterval;
		if (((m_rcItem.bottom - m_rcItem.top) % m_nVerWarterMarkInterval) != 0)
		{
			nVerCount = nVerCount + 1;
		}
		int nlasttop = m_rcItem.top;
		for (int i = 0; i < nHorCount*nVerCount; i++)                              //计算每个单位矩形区域绘制文字
		{
			UiRect rc(0, 0, 0, 0);
			if (i% nHorCount == 0 && i != 0)
			{
				rc.left = m_rcItem.left;
				rc.top = nlasttop + m_nVerWarterMarkInterval;
			}
			else
			{
				rc.left = m_rcItem.left + i% nHorCount * m_nHorWarterMarkInterval;
				rc.top = nlasttop;
			}
			nlasttop = rc.top;
			rc.right = rc.left + m_nHorWarterMarkInterval;
			rc.bottom = rc.top + m_nVerWarterMarkInterval;
			DWORD dwClrColor;
			if (m_strWaterMarkColor.empty())
			{
				dwClrColor = 0xff020f22;
			}
			else
			{
				dwClrColor = GlobalManager::GetTextColor(m_strWaterMarkColor);
			}
			pRender->DrawText(rc, m_strWarterMarkText, dwClrColor, 1, DT_CENTER | DT_VCENTER, 16, false, true);    //百分之六的透明度
		}
	}
}

void Control::SetWaterMarkText(const std::wstring& strText)
{
	m_strWarterMarkText = strText;
}

void Control::GetWaterMarkText(std::wstring& strText)
{
	strText = m_strWarterMarkText;
}

void Control::SetWaterMarkHorInterval(unsigned int interval)
{
	m_nHorWarterMarkInterval = interval;
}

void Control::SetWaterMarkVerInterval(unsigned int interval)
{
	m_nVerWarterMarkInterval = interval;
}

void Control::SetWaterMarkEnable(bool bEnable)
{
	m_EnableWaterMark = bEnable;
}

void Control::SetWaterMarkColor(const std::wstring& strTextColor)
{
	m_strWaterMarkColor = strTextColor;
}
...
void Control::SetRandHeight(std::vector<int> vecHeightValue)
{
	m_vecRandHeight = vecHeightValue;
}

void Control::SetRandHeightbottomMargin(int nValue)
{
	m_RandbottomMargin = nValue;
}
...
void Control::StopGifPlay(int frame)
{
...
void Control::StartGifPlayForUI(int frame, int playcount)
{
	GetImage(m_bkImage);
...
void Control::StopGifPlayForUI(bool transfer, int frame)
{
	m_bGifPlay = false;
	StopGifPlay(frame);
	if (transfer)
...
int Control::GetGifFrameIndex(int frame)
{
	int ret = frame;
	switch (frame)
	{
...
void Control::InvokeLoadImageCache()
{
	...
	if (!m_bkImage.imageCache || m_bkImage.imageCache->sImageFullPath != imageFullPath) {
		...
		std::function<std::shared_ptr<ImageInfo>()> getImageCallback = [this, imageFullPath]() {
			return std::shared_ptr<ImageInfo>(ImageInfo::LoadImage(imageFullPath).release(), &GlobalManager::OnImageInfoDestroy);
		};
		getImageCallback = m_loadBkImageWeakFlag.ToWeakCallback(getImageCallback);
		std::function<void(const std::shared_ptr<ImageInfo>&)> setImagecallback = [this](const std::shared_ptr<ImageInfo>& shared_image) {
			if (m_loadBkImageWeakFlag.HasUsed() && shared_image) {
				auto cache_shared_image = GlobalManager::IsImageCached(shared_image->sImageFullPath);
				if (cache_shared_image) {
					m_bkImage.imageCache = cache_shared_image;
					shared_image->sImageFullPath = L"";
				}
				else {
					m_bkImage.imageCache = GlobalManager::AddImageCached(shared_image);
				}
				m_loadBkImageWeakFlag.Cancel();
			}
		};
		setImagecallback = ToWeakCallback(setImagecallback);
		nbase::ThreadManager::Await(ThreadId::kThreadDuilibHelper, getImageCallback, setImagecallback);
	}
}
