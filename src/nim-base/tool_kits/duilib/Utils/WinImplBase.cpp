...
// 		&& pt.y >= rcClient.top + rcCaption.top && pt.y < rcClient.top + rcCaption.bottom ) {
	if ((pt.x >= rcClient.left && pt.x < rcClient.left + rcCaption.left) ||
		(pt.x >= rcClient.right - rcCaption.right && pt.x < rcClient.right) ||
		(pt.y <= rcClient.top + rcCaption.top && pt.y > rcClient.top) ||
		(pt.y <= rcClient.bottom && pt.y > rcClient.bottom - rcCaption.bottom)) {
...
LRESULT WindowImplBase::OnMButtonDown(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	bHandled = FALSE;
	return 0;
}

LRESULT WindowImplBase::OnMButtonUp(UINT /*uMsg*/, WPARAM /*wParam*/, LPARAM /*lParam*/, BOOL& bHandled)
{
	bHandled = FALSE;
	return 0;
}
...
LRESULT WindowImplBase::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MBUTTONUP:		lRes = OnMButtonUp(uMsg, wParam, lParam, bHandled); break;
	case WM_MBUTTONDOWN:	lRes = OnMButtonDown(uMsg, wParam, lParam, bHandled); break;
