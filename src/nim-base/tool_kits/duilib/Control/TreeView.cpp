...
void TreeNode::SetWindow(Window* pManager, Box* pParent, bool bInit)
{
	...
	{
		(*it)->SetWindow(pManager, pParent, bInit);
	}
...
{
	if (strName == _T("expandimage"))
	{
		SetExpandImage(strValue);
	}
	else if (strName == _T("collapseimage"))
	{
		SetCollapseImage(strValue);
	}
	else
	{
...
void TreeNode::PaintStatusImage(IRenderContext* pRender)
{
	__super::PaintStatusImage(pRender);

	PaintExpandImage(pRender);
}

void TreeNode::PaintExpandImage(IRenderContext* pRender)
{
	if (0 >= GetDepth())
	{
		if (IsExpand())
		{
			if (!m_imgExpand.imageAttribute.simageString.empty())
			{
				DrawImage(pRender, m_imgExpand);
			}
		}
		else
		{
			if (!m_imgCollapse.imageAttribute.simageString.empty())
			{
				DrawImage(pRender, m_imgCollapse);
			}
		}
	}
	else
	{
		if (IsExpand())
		{
			if (!m_imgExpand.imageAttribute.simageString.empty())
			{
				const ImageAttribute& imgAttr = m_imgExpand.imageAttribute;
				int iDest = m_iDepth * m_pTreeView->GetIndent();
				std::wstring strModify = StringHelper::Printf(L"source='%d,%d,%d,%d' dest='%d,%d,%d,%d'",
					imgAttr.rcSource.left,
					imgAttr.rcSource.top,
					imgAttr.rcSource.right,
					imgAttr.rcSource.bottom,
					imgAttr.rcDest.left + iDest,
					imgAttr.rcDest.top,
					imgAttr.rcDest.right + iDest,
					imgAttr.rcDest.bottom);
				DrawImage(pRender, m_imgExpand, strModify);
			}
		}
		else
		{
			if (!m_imgCollapse.imageAttribute.simageString.empty())
			{
				...
				std::wstring strModify = StringHelper::Printf(L"source='%d,%d,%d,%d' dest='%d,%d,%d,%d'",
					...
					imgAttr.rcDest.right + iDest,
					imgAttr.rcDest.bottom);
				DrawImage(pRender, m_imgCollapse, strModify);
			}
		}
	}
}

std::wstring TreeNode::GetExpandImage(void) const
{
	return m_imgExpand.imageAttribute.simageString;
}

void TreeNode::SetExpandImage(const std::wstring& strImage)
{
	m_imgExpand.SetImageString(strImage);
	Invalidate();
}

std::wstring TreeNode::GetCollapseImage(void) const
{
	return m_imgCollapse.imageAttribute.simageString;
}

void TreeNode::SetCollapseImage(const std::wstring& strImage)
{
	m_imgCollapse.SetImageString(strImage);
	Invalidate();
}
...
{
	TreeNode *pNode = dynamic_cast<ui::TreeNode*>(pControl);
	if (NULL != pNode)
	{
		return AddChildNode(pNode);
	}
	else
	{
...
bool TreeNode::AddChildNodeAt(TreeNode* pTreeNode, std::size_t iIndex)
{
	...
	if (m_iDepth != ROOT_NODE_DEPTH)
	{
		UiRect rcTextPadding = pTreeNode->GetTextPadding();
		rcTextPadding.left += m_pTreeView->GetIndent() * pTreeNode->m_iDepth;
		pTreeNode->SetTextPadding(rcTextPadding);
	}
...
bool TreeView::Add(Control* pControl)
{
	...
	if (NULL != pNode)
	{
		return GetRootNode()->AddChildNode(pNode);
	}
	else
	{
		ASSERT(FALSE);
...
void TreeView::SetWindow(Window* pManager, Box* pParent, bool bInit)
{
	...
	m_rootNode->SetWindow(pManager, this, bInit);
}
