...
LabelTemplate<InheritType>::LabelTemplate() :
    ...
    m_bSingleLine(true),
    m_bLineLimit(false),
	m_bMeasureTrailingSpaces(false),
	m_hAlign(DT_LEFT),
	m_vAlign(DT_CENTER),
	m_rcTextPadding(),
	m_sText(),
...
CSize LabelTemplate<InheritType>::EstimateText(CSize szAvailable, bool& bReEstimateSize)
{
	...
	if (!GetText().empty()) {
		...
		UiRect rect = pRender->MeasureText(GetText(), m_iFont, m_uTextStyle, IsMeasureTrailingSpaves(), width);
		...
		if (GetFixedHeight() == DUI_LENGTH_AUTO) {
			...
			if (!m_bSingleLine && GetFixedWidth() == DUI_LENGTH_AUTO && GetMaxWidth() == DUI_LENGTH_STRETCH) {
				...
				if (estimateWidth > maxWidth) {
					...
					UiRect newRect = pRender->MeasureText(GetText(), m_iFont, m_uTextStyle, IsMeasureTrailingSpaves(), estimateWidth);
...
void LabelTemplate<InheritType>::SetAttribute(const std::wstring& strName, const std::wstring& strValue)
{
	...
	else if (strName == _T("measuretrailingspaces")) SetMeasureTrailingSpaces(strValue == _T("true"));
...
template<typename InheritType>
bool LabelTemplate<InheritType>::IsMeasureTrailingSpaves(void) const
{
	return m_bMeasureTrailingSpaces;
}

template<typename InheritType>
void LabelTemplate<InheritType>::SetMeasureTrailingSpaces(bool bMeasure)
{
	if (m_bMeasureTrailingSpaces == bMeasure) return;

	m_bMeasureTrailingSpaces = bMeasure;
	Invalidate();
}
