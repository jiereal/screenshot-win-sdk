...
public:
	...
	virtual INT_PTR GetTag(void) const;
	virtual void SetTag(INT_PTR iTag);
	...
	virtual void PaintWaterMark(IRenderContext* pRender);

	void SetWaterMarkText(const std::wstring& strText);
	void GetWaterMarkText(std::wstring& strText);
	void SetWaterMarkHorInterval(unsigned int interval);
	void SetWaterMarkVerInterval(unsigned int interval);
	void SetWaterMarkEnable(bool bEnable);
	void SetWaterMarkColor(const std::wstring& strTextColor);
	...
	//设置图片在控件上随机值高度显示
	void SetRandHeight(std::vector<int> vecHeightValue);
	void SetRandHeightbottomMargin(int nValue);
...
protected:
	...
	std::vector<int>  m_vecRandHeight;
	int               m_RandbottomMargin;

	bool  m_EnableWaterMark;                       //水印相关属性
	std::wstring m_strWarterMarkText;
	unsigned int m_nHorWarterMarkInterval;        //水印水平间距
	unsigned int m_nVerWarterMarkInterval;        //水印垂直间距
	std::wstring m_strWaterMarkColor;
