...
public:
	...
	virtual Control* GetItemByName(const std::wstring& strName);
	virtual Control* GetItemByUTF8Name(const std::string& strName);
	virtual Control* GetItemByDataID(const std::wstring& strText);
	virtual Control* GetItemByUTF8DataID(const std::string& strText);
	//可以item DataId可以允许相同
	virtual void GetItemByUTF8DataID(const std::string& strText, std::list<Control*>& m_listItem);    
	...
	int GetSepSize(void) const;
	void SetSepSize(int nSize);
	int GetSepType(void) const;
	void SetSepType(int nType);
...
private:
	...
	bool IsInSepArea(SepType eSepType, const POINT& ptMouse);
	bool IsInSepHorArea(const POINT& ptMouse);
	bool IsInSepVerArea(const POINT& ptMouse);
	int GetSepWidth(void) const;
	int GetSepHeight(void) const;
	void DragSep(const POINT& ptMouse);

	EventMap OnXmlBubbledEvent;

protected:
	std::unique_ptr<Layout> m_pLayout;
	int m_nSepSize;						// 分隔符大小
	int m_nSepType;						// 分隔符类型
	int m_nSepTypeDrag;					// 拖动分隔符类型
	bool m_bAutoDestroy;
	bool m_bDelayedDestroy;
	bool m_bMouseChildEnabled;
	ControlStateType m_uSepState;		// 分隔符状态
