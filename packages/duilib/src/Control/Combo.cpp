...
void CComboWnd::Init(Combo* pOwner)
{
	...
	// VBox 默认的Padding 调整
	cyFixed += pOwner->GetListBox()->GetLayout()->GetPadding().top + pOwner->GetListBox()->GetLayout()->GetPadding().bottom;
	...
	m_sDropBoxAttributes()
{
	...
	m_pLayout->SetBorderColor(L"combobox_border2");
...
void Combo::Activate()
{
	...
	{
		auto item = m_pLayout->GetItemAt(i);
		if (item)
		{
			item->SetState(kControlStateNormal);
			item->Invalidate();
		}
	}
...
void Combo::SetDropBoxAttributeList(const std::wstring& pstrList)
{
	...
	if (NULL != m_pLayout && !pstrList.empty())
	{
...
ui::Control* Combo::GetItemByName(const std::wstring& strText)
{
	return m_pLayout->GetItemByName(strText);
}

ui::Control* Combo::GetItemByDataID(const std::wstring& strText)
{
	return m_pLayout->GetItemByDataID(strText);
}

ui::Control* Combo::GetItemByUTF8DataID(const std::string& strText)
{
	return m_pLayout->GetItemByUTF8DataID(strText);
}

int Combo::GetItemIndex(Control* pControl) const
{
	return m_pLayout->GetItemIndex(pControl);
}
