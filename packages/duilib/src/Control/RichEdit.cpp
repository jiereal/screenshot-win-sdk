...
BOOL CTxtWinHost::DoSetCursor(UiRect *prc, POINT *pt)
{
    ...
    if (PtInRect(&rc, newPt))
    {
		// 判断是否在自定义链接处
		std::wstring strInfo;
		if (m_re->HittestCustomLink(*pt, strInfo))
		{
			return TRUE;
		}
...
WCHAR RichEdit::SetPasswordChar(WCHAR chPasswordChar)
{
	if (m_pTwh)
	{
		return m_pTwh->SetPasswordChar(chPasswordChar);
	}
	return L'*';
}
...
void RichEdit::SetPromptFont(int index)
{
	m_iPromptFont = index;
}
...
HRESULT RichEdit::TxSendMessage(UINT msg, WPARAM wParam, LPARAM lParam, LRESULT *plresult) const
{
    if( m_pTwh ) {
		...
		else if (msg == WM_MOUSEWHEEL && ::GetAsyncKeyState(VK_CONTROL) < 0)
			return S_OK;
...
void RichEdit::Paint(IRenderContext* pRender, const UiRect& rcPaint)
{
	...
	ui::UiRect rcRestoreAlpha = rcTemp;
    ...
    if( m_pTwh ) {
		...
		ptOrg.x = -ptOrg.x;
		ptOrg.y = -ptOrg.y;
		rcRestoreAlpha.Offset(ptOrg);

		UiRect rcRootPadding;
		Box *pBoxRoot = dynamic_cast<Box*>(GetWindow()->GetRoot());
		if (NULL != pBoxRoot)
		{
			rcRootPadding = pBoxRoot->GetLayout()->GetPadding();
			//考虑圆角
		...
		pRender->RestoreAlpha(rcRestoreAlpha, rcRootPadding);
#endif
...
void RichEdit::SetAttribute(const std::wstring& strName, const std::wstring& strValue)
{
	...
	else if (strName == _T("promptfont"))	SetPromptFont(_ttoi(strValue.c_str()));
	...
	else if (strName == _T("limit")) SetLimitText(_ttoi(strValue.c_str()));
...
bool RichEdit::IsPromptMode(void) const
{
...
void RichEdit::PaintPromptText(IRenderContext* pRender)
{
	...
	int ifont = m_iPromptFont >= 0 ? m_iPromptFont : m_iFont;
	...
	pRender->DrawText(rc, strPrompt, dwClrColor, ifont, dwStyle);
}
...
bool RichEdit::HittestCustomLink(CPoint pt, std::wstring& info)
{
	...
	info.clear();
	pt.Offset(GetScrollOffset());
	if (m_linkInfo.size() > 0 && GetPos().IsPointIn(pt))
	{
