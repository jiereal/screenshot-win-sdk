...
public:
	...
	virtual void DrawImage(const UiRect& rcPaint, HBITMAP hBitmap, bool bAlphaChannel,
		const UiRect& rcImageDest, const UiRect& rcImageSource, const UiRect& rcCorners, BYTE uFade = 255, bool xtiled = false, bool ytiled = false, int nRandBottomMargin = 0 , std::vector<int> vecRandHeight = std::vector<int>()) = 0;
	...
	virtual void DrawText(const UiRect& rc, const std::wstring& strText, DWORD dwTextColor, int iFont, UINT uStyle, BYTE uFade = 255, bool bLineLimit = false, bool bWaterMark = false) = 0;

	virtual UiRect MeasureText(const std::wstring& strText, int iFont, UINT uStyle, int width = DUI_NOSET_VALUE) = 0;
	virtual UiRect MeasureText(const std::wstring& strText, int iFont, UINT uStyle, bool bMeasureTrailingSpaces, int width = DUI_NOSET_VALUE) = 0;
};
