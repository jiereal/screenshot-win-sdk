...
static Local<Value> get_object_value(Isolate* isolate, const Local<Object>& obj, const utf8_string& keyName)
{
    auto name = nim_napi_new_utf8string(isolate, keyName.c_str());
    return obj->Get(isolate->GetCurrentContext(), name).ToLocalChecked();
}

napi_status nim_napi_get_object_value(Isolate* isolate, const Local<Object>& obj, const utf8_string& keyName, Local<Value>& out)
{
    out = get_object_value(isolate, obj, keyName);
    if (!out.IsEmpty())
        return napi_ok;
    return napi_invalid_arg;
}
...
{
    auto value = get_object_value(isolate, obj, keyName);
    return nim_napi_get_value_utf8string(isolate, value, out);
}
