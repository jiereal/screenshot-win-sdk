...
void CCaptureEdit::DrawExtraUnits(HDC hdc, bool bScaled)
{
	if (extra_bitmap_ &&!bScaled)
	{
	...
	else
	{
		for (size_t i = 0; i < vec_draw_units_.size(); i++)
		{
			...
			vec_draw_units_[i]->Render(hdc, blur_bitmap_,false, bScaled);
		}
		if (vec_draw_units_.size() > 0 && !bScaled)
		{
			GetExtraUnitsBimap(hdc, extra_bitmap_);
		}
	}

	if (current_draw_unit_&&!bScaled)
	{
