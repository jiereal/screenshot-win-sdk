...
void CaptureWindow::InitWindow()
{
	...
	if (!ui::DpiManager::GetInstance()->IsAdaptDPI())
	{
		...
		g_DesktopScale = dpi_value * 100 / 96;
	}
	...
	ShowToolBar();
	ShowSizeTip();
	Label *ctl_size_tip = static_cast<Label *>(FindControl(L"size_tip"));
	if (ctl_size_tip)
	{
		ctl_size_tip->SetVisible(true);
	}
...
BOOL CaptureWindow::SaveTrackImage(bool save_to_clipboard)
{
	...
	if (bitmap_bg_ && DeTrackState(track_state_) == TRACKED)
	{
		...
		x = x * g_DesktopScale / 100;
		y = y * g_DesktopScale / 100;
		width = width * g_DesktopScale / 100;
		height = height * g_DesktopScale / 100;
		...
		if (capture_edit_)
		{
			bool bScaled = g_DesktopScale != 100;
			capture_edit_->DrawExtraUnits(hScrDC2, bScaled);
		}
...
{
	BOOL bResult = FALSE;
	int x = screen_x_ * g_DesktopScale / 100;
	int y = screen_y_ * g_DesktopScale / 100;
	int width = screen_width_ * g_DesktopScale / 100;
	int height = screen_height_ * g_DesktopScale / 100;
...
RECT GetMonitorWorkRectByPos(LONG lX, LONG lY)
{
	RECT rcWork = { 0, 0, 0, 0 };
	// 右下点
	ui::CPoint ptLowRight(lX, lY);
	HMONITOR hMonitor = ::MonitorFromPoint(ptLowRight, MONITOR_DEFAULTTONEAREST);
	if (NULL == hMonitor)
		return rcWork;
	...
	return stMonitorInfo.rcWork;
}

RECT GetMonitorWorkRectByRect(const RECT& rcPos)
{
	RECT rcWork = { 0, 0, 0, 0 };
	HMONITOR hMonitor = ::MonitorFromRect(&rcPos, MONITOR_DEFAULTTONEAREST);
	if (NULL == hMonitor)
		return rcWork;
...
RECT ClientRectToScreenRect(HWND hWnd, RECT rcClient)
{
	...
	ui::UiRect rcScreen;
	rcScreen.left = ptLeftTop.x;
	rcScreen.top = ptLeftTop.y;
	rcScreen.right = ptRightBottom.x;
	rcScreen.bottom = ptRightBottom.y;
...
{
	RECT rcClipTrack = ClipTrackRect(track_rect_);
	int width = rcClipTrack.right - rcClipTrack.left;
	...
	int iVerticalBoarder = boarder * 2;
	DpiManager::GetInstance()->ScaleInt(iVerticalBoarder);

	ui::UiRect rcScreenClipTrack = ClientRectToScreenRect(m_hWnd, rcClipTrack);
	RECT rcWork = GetMonitorWorkRectByPos(rcScreenClipTrack.right, rcScreenClipTrack.bottom);

	int x = 0;
	if (rcScreenClipTrack.right - width < rcWork.left)
	{
		x = rcClipTrack.right - (rcScreenClipTrack.right - rcWork.left);
		width = toolbar_min_width_;
	}
	else
	{
		x = rcClipTrack.right - width;
	}
	...
	int y = rcClipTrack.bottom + iVerticalBoarder;
	bool bIsBottomExceed = (rcScreenClipTrack.bottom + height + iVerticalBoarder > rcWork.bottom);
	bool bIsTopExceed = (rcScreenClipTrack.top - height - iVerticalBoarder < rcWork.top);
	if (bIsTopExceed && bIsBottomExceed)
	{
		y = rcClipTrack.bottom - height - iVerticalBoarder;
	}
	else if (bIsBottomExceed)
	{
		y = rcClipTrack.top - height - iVerticalBoarder;
		if (y < rcWork.top)
		{
			y = rcClipTrack.top + boarder;
		}
	}
	else if (bIsTopExceed)
	{
		y = rcClipTrack.bottom + boarder;
		if (y > rcWork.bottom)
		{
			y = rcClipTrack.bottom - height - iVerticalBoarder;
		}
	...
	rcClipTrack.left = x;
	rcClipTrack.top = y;
	rcClipTrack.right = x + width;
	rcClipTrack.bottom = y + height;
	return rcClipTrack;
}
...
{
	if (capture_edit_mode_ == mode && is_show_color_palette_ && mode > EM_NONE)
	{
		// 重复点击相同的编辑单元，那么收起调色板
		is_show_color_palette_ = false;
	}
