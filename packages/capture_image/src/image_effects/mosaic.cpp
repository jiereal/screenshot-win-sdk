...
namespace ImageEffects
{
	void GetBlockColorMean(unsigned long &color, int real_width, int dest_width, int dest_height, long* data_start, int step)
	{
		...
		*point_clr++ = (unsigned char)B_t;
		*point_clr++ = (unsigned char)G_t;
		*point_clr++ = (unsigned char)R_t;
	}
	...
	void ChangeRandomColor(unsigned long &color, int pt_num)
	{
		...
		int r2 = rand();
		int r3 = rand();
		point_clr[0] = (unsigned char)MIX_CLR(point_clr[0], r1);
		point_clr[1] = (unsigned char)MIX_CLR(point_clr[1], r2);
		point_clr[2] = (unsigned char)MIX_CLR(point_clr[2], r3);
	}
